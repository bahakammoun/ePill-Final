...
import com.doccuty.epill.activesubstance.ActiveSubstanceRepository;
...
import com.doccuty.epill.healthcondition.DiseaseGroupRepository;
...
public class UserService {
	...
	@Autowired
	DrugFeatureRepository drugFeatureRepository;

	@Autowired
	ActiveSubstanceRepository activeSubstanceRepository;

	@Autowired
	DiseaseGroupRepository diseaseGroupRepository;
	...
	private User saveUserPreferences(User user, User usr) {

		Set<DiseaseGroup> diseases = usr.getSufferedDiseaseGroup();
		Set<DiseaseGroup> diseaseGroups = new HashSet<>();
		if (user.getSufferedDiseaseGroup() != null)
			user.withoutSufferedDiseaseGroup(user.getSufferedDiseaseGroup().toArray(new DiseaseGroup[user.getSufferedDiseaseGroup().size()]));
		if (diseases != null) {
			for (DiseaseGroup diseaseGroup : diseases) {
				DiseaseGroup diseaseGroup1 = diseaseGroupRepository.findOne(diseaseGroup.getId());
				diseaseGroups.add(diseaseGroup1);
				diseaseGroup1.withUserSuffering(user);
			}
			user.setSufferedDiseaseGroup(diseaseGroups);
		}


		Set<PharmaceuticalForm> pharmaceuticalForms = usr.getPreferredPharmaceuticalForms();
		Set<PharmaceuticalForm> preferredPharmaceuticalForms = new HashSet<>();
		if (user.getPreferredPharmaceuticalForms() != null)
			user.withoutPharmaceuticalForm(user.getPreferredPharmaceuticalForms().toArray(new PharmaceuticalForm[user.getPreferredPharmaceuticalForms().size()]));
		if (pharmaceuticalForms != null) {
			for (PharmaceuticalForm pharmaceuticalForm : pharmaceuticalForms) {
				PharmaceuticalForm pharmaceuticalForm1 = pharmaceuticalFormRepository.findOne(pharmaceuticalForm.getId());
				preferredPharmaceuticalForms.add(pharmaceuticalForm1);
				pharmaceuticalForm1.withUserPreferring(user);
			}
			user.setPreferredPharmaceuticalForms(preferredPharmaceuticalForms);
		}

		Set<DrugFeature> drugFeatures = usr.getPreferredDrugFeature();
		Set<DrugFeature> preferredDrugFeatures = new HashSet<>();
		if (user.getPreferredDrugFeature() != null)
			user.withoutPreferredDrugFeature(user.getPreferredDrugFeature().toArray(new DrugFeature[user.getPreferredDrugFeature().size()]));
		if (drugFeatures != null) {
			for (DrugFeature drugFeature : drugFeatures) {
				DrugFeature drugFeature1 = drugFeatureRepository.findOne(drugFeature.getId());
				preferredDrugFeatures.add(drugFeature1);
				drugFeature1.setUser(user);
			}
			user.setPreferredDrugFeatures(preferredDrugFeatures);
		}

		Set<ActiveSubstance> activeSubstances = usr.getAllergicSubstances();
		Set<ActiveSubstance> allergicSubstances = new HashSet<>();
		if (user.getAllergicSubstances() != null)
			user.withoutAllergicSubstance(user.getAllergicSubstances().toArray(new ActiveSubstance[user.getAllergicSubstances().size()]));
		if (activeSubstances != null) {
			for (ActiveSubstance activeSubstance : activeSubstances) {
				ActiveSubstance activeSubstance1 = activeSubstanceRepository.findOne(activeSubstance.getId());
				allergicSubstances.add(activeSubstance1);
				activeSubstance1.setUser(user);
			}
			user.setAllergicSubstances(allergicSubstances);
		}
	...
	public User saveUser(User user) {
		...
		Set<DiseaseGroup> diseases = user.getSufferedDiseaseGroup();
		Set<DiseaseGroup> diseaseGroups = new HashSet<>();
		if (diseases != null) {
			for (DiseaseGroup diseaseGroup : diseases) {
				DiseaseGroup diseaseGroup1 = diseaseGroupRepository.findOne(diseaseGroup.getId());
		...
		Set<PharmaceuticalForm> pharmaceuticalForms = user.getPreferredPharmaceuticalForms();
		Set<PharmaceuticalForm> preferredPharmaceuticalForms = new HashSet<>();
		if (pharmaceuticalForms != null) {
			for (PharmaceuticalForm pharmaceuticalForm : pharmaceuticalForms) {
				PharmaceuticalForm pharmaceuticalForm1 = pharmaceuticalFormRepository.findOne(pharmaceuticalForm.getId());
		...
		Set<DrugFeature> drugFeatures = user.getPreferredDrugFeature();
		Set<DrugFeature> preferredDrugFeatures = new HashSet<>();
		if (drugFeatures != null) {
			for (DrugFeature drugFeature : drugFeatures) {
				DrugFeature drugFeature1 = drugFeatureRepository.findOne(drugFeature.getId());
		...
		Set<ActiveSubstance> activeSubstances = user.getAllergicSubstances();
		Set<ActiveSubstance> allergicSubstances = new HashSet<>();
		if (activeSubstances != null) {
			for (ActiveSubstance activeSubstance : activeSubstances) {
				ActiveSubstance activeSubstance1 = activeSubstanceRepository.findOne(activeSubstance.getId());
	...
	public User updateUserData(User usr) {
		...
		user.withSize(usr.getSize())
				.withWeight(usr.getWeight())
		...
		this.saveUserPreferences(user, usr);
	...
	public User saveDiseaseInvocation(DiseaseInvocation click) {
		User user = repository.findOne(click.getUser().getId());
